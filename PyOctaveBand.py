...
def octavefilter(x, fs, fraction=1, order=6, limits=None, show=0, sigbands=0):
    ...
    if sigbands:
        ...
        for idx in range(len(freq)):
            sd = sp.signal.decimate(x, factor[idx])
            y = sp.signal.sosfilt(sos[idx], sd)
            spl[idx] = 20 * np.log10(np.std(y) / 2e-5)
            xb.append(sp.signal.resample_poly(y, factor[idx], 1))
...
def _buttersosfilter(freq, freq_d, freq_u, fs, order, factor, show=0):
    ...
    for idx, (lower, upper) in enumerate(zip(freq_d, freq_u)):
        ...
        sos[idx] = sp.signal.butter(N=order,
...
def _showfilter(sos, freq, freq_u, freq_d, fs, factor):
    ...
    for idx in range(len(freq)):
        fsd = fs / factor[idx]  # New sampling rate
        w[:, idx], h[:, idx] = sp.signal.sosfreqz(sos[idx],
                                                  worN=wn,
                                                  whole=False,
                                                  fs=fsd)
    ...
    ax.set_xticklabels([
        ...
        '250', '315', '400', '500', '630', '800', '1k', '12.5k', '16k', '2k',
        '2.5k', '31.5k', '4k', '5k', '6.3k', '8k', '10k', '12.5k', '16k',
        '20k', '25k'
    ])
    plt.show()
